...
void *core_sysdep_malloc(uint32_t size, char *name) { return malloc(size); }

void core_sysdep_free(void *ptr) { free(ptr); }
...
void core_sysdep_sleep(uint64_t time_ms) { usleep(time_ms * 1000); }
...
static int32_t _core_sysdep_network_tcp_establish(core_network_handle_t *network_handle)
{
    ...
    res = _core_sysdep_network_connect(network_handle->host, network_handle->port, AF_INET, SOCK_STREAM, IPPROTO_IP,
                                       network_handle->connect_timeout_ms, &network_handle->fd);
    if (res == STATE_PORT_NETWORK_DNS_FAILED && strlen(network_handle->backup_ip) > 0) {
        printf("using backup ip: %s\n", network_handle->backup_ip);
        res = _core_sysdep_network_connect(network_handle->backup_ip, network_handle->port, AF_INET, SOCK_STREAM, IPPROTO_IP,
                                           network_handle->connect_timeout_ms, &network_handle->fd);
    }
...
static int32_t _core_sysdep_network_mbedtls_establish(core_network_handle_t *network_handle)
{
    ...
    if (network_handle->socket_type == CORE_SYSDEP_SOCKET_TCP_CLIENT) {
        res = _core_sysdep_network_connect(network_handle->host, network_handle->port, AF_INET, SOCK_STREAM, IPPROTO_IP,
                                           network_handle->connect_timeout_ms, &network_handle->mbedtls.net_ctx.fd);
    } else if (network_handle->socket_type == CORE_SYSDEP_SOCKET_UDP_CLIENT) {
        res = _core_sysdep_network_connect(network_handle->host, network_handle->port, AF_UNSPEC, SOCK_DGRAM, IPPROTO_UDP,
                                           network_handle->connect_timeout_ms, &network_handle->mbedtls.net_ctx.fd);
    }
    ...
    mbedtls_ssl_conf_max_version(&network_handle->mbedtls.ssl_config, MBEDTLS_SSL_MAJOR_VERSION_3, MBEDTLS_SSL_MINOR_VERSION_3);
    mbedtls_ssl_conf_min_version(&network_handle->mbedtls.ssl_config, MBEDTLS_SSL_MAJOR_VERSION_3, MBEDTLS_SSL_MINOR_VERSION_3);
    ...
    if (network_handle->cred->option == AIOT_SYSDEP_NETWORK_CRED_SVRCERT_CA) {
        ...
        res = mbedtls_ssl_conf_psk(&network_handle->mbedtls.ssl_config, (const unsigned char *)network_handle->psk.psk,
                                   (size_t)strlen(network_handle->psk.psk), (const unsigned char *)network_handle->psk.psk_id,
    ...
    mbedtls_ssl_set_bio(&network_handle->mbedtls.ssl_ctx, &network_handle->mbedtls.net_ctx, mbedtls_net_send, mbedtls_net_recv,
    ...
    printf("success to establish mbedtls connection, fd = %d(cost %d bytes in total, max used %d bytes)\n",
           (int)network_handle->mbedtls.net_ctx.fd, g_mbedtls_total_mem_used, g_mbedtls_max_mem_used);
                                             ...
                                             uint32_t timeout_ms)
{
    ...
    do {
        ...
        if (timenow_ms - timestart_ms >= timenow_ms || timeout_ms - (timenow_ms - timestart_ms) > timeout_ms) {
            break;
        }
...
int32_t _core_sysdep_network_tcp_send(core_network_handle_t *network_handle, uint8_t *buffer, uint32_t len, uint32_t timeout_ms)
{
    int res               = 0;
...
int32_t _core_sysdep_network_udp_send(core_network_handle_t *network_handle, uint8_t *buffer, uint32_t len, uint32_t timeout_ms,
